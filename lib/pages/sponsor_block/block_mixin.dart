...
mixin BlockMixin on GetxController {
  ...
  void _skipToast(SegmentModel item) {
    if (autoPlay && Pref.blockToast) {
      _showBlockToast(
        'sponsor_block.skipped_segment'.trParams({
          'var0': (item.segmentType.shortTitle).toString(),
        }),
      );
    }
  ...
  Future<void> onSkip(
    ...
    try {
      ...
      if (isSkip) {
        ...
        _showBlockToast(
          'sponsor_block.jumped_to'.trParams({
            'var0': (item.segmentType.shortTitle).toString(),
          }),
        );
      }
      ...
      if (isSkip) {
        _showBlockToast(
          'sponsor_block.failed_to_skip_segment'.trParams({
            'var0': (item.segmentType.shortTitle).toString(),
          }),
        );
      } else {
  ...
  void _doVote(String uuid, int type) =>
      SponsorBlock.voteOnSponsorTime(
        ...
        (i) => SmartDialog.showToast(
          i.isSuccess
              ? 'sponsor_block.voted_successfully'.tr
              : 'sponsor_block.vote_failed'.trParams({'var0': (i).toString()}),
        ),
      );
