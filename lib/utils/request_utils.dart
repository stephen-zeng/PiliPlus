    ...
    if (!isFollow) {
      ...
      if (context.mounted) {
        ...
        String text = isSpecialFollowed
            ? 'request.remove_special'.tr
            : 'request.add_special'.tr;
        showDialog(
          context: context,
  ...
  static Future<void> onLikeDynamic(
    ...
    if (status ^ uiStatus) {
      SmartDialog.showToast(
        status ? 'request.like_success'.tr : 'request.cancel_like'.tr,
      );
      onSuccess();
      return;
    }
    ...
    if (res.isSuccess) {
      SmartDialog.showToast(
        status ? 'request.cancel_like'.tr : 'request.like_success'.tr,
      );
      like
                    ...
                    if (checkedId != null) {
                      ...
                      FavHttp.copyOrMoveFav(
                        ...
                        if (res.isSuccess) {
                          ...
                          SmartDialog.showToast(
                            isCopy
                                ? 'request.copy_success'.tr
                                : 'request.move_success'.tr,
                          );
                          Get.back();
                        } else {
                          SmartDialog.dismiss();
